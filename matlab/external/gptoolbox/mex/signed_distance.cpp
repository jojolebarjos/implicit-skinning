...
//   -IC:/Users/tkach/OneDrive/EPFL/Code/External/libigl/include ...
//   -IC:/Users/tkach/OneDrive/EPFL/Code/External/eigen_dir ...
//   signed_distance.cpp
  ...
  igl::SignedDistanceType & type)
{
  ...
  igl::matlab::mexErrMsgTxt(nrhs >= 3, "The number of input arguments must be >=3.");

  const int dim = mxGetN(prhs[0]);
  igl::matlab::mexErrMsgTxt(dim == 3,
  ...
  igl::matlab::mexErrMsgTxt(dim == mxGetN(prhs[2]),
    "Mesh \"face\" simplex size must equal dimension");

  igl::matlab::parse_rhs_double(prhs,P);
  igl::matlab::parse_rhs_double(prhs+1,V);
  igl::matlab::parse_rhs_index(prhs+2,F);
  ...
  type = SIGNED_DISTANCE_TYPE_PSEUDONORMAL;
  {
    ...
    while(i<nrhs)
    {
      igl::matlab::mexErrMsgTxt(mxIsChar(prhs[i]),"Parameter names should be strings");
      // Cast to char
      ...
      if(strcmp("SignedDistanceType",name) == 0)
      {
        ...
        {
          igl::matlab::mexErrMsgTxt(false,C_STR("Unknown SignedDistanceType: "<<type_name));
        }
      }else
      {
        igl::matlab::mexErrMsgTxt(false,"Unknown parameter");
      }
      i++;
    }
  }
  ...
  int nrhs, const mxArray *prhs[])
{
  ...
  switch(nlhs)
  {
    default:
    {
      igl::matlab::mexErrMsgTxt(false,"Too many output parameters.");
    }
    case 4:
    {
      igl::matlab::prepare_lhs_double(N,plhs+3);
      // Fall through
    }
    case 3:
    {
      igl::matlab::prepare_lhs_double(C,plhs+2);
      // Fall through
    }
    case 2:
    {
      igl::matlab::prepare_lhs_index(I,plhs+1);
      // Fall through
    }
    case 1:
    {
