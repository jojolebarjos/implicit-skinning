function [lambda,W,tan_W] = mvc(V,C)
  ...
  if((size(C,2) == 2 || C_flat) && (size(V,2) == 2 || C_flat))
    ...
    % This assert is biaised since a poly with more vertices will hit the threshold just by summing numerical errors
%     assert(sqrt(sum(dist_to_plane.^2))<1e-5); 
    dist_to_plane = dot(repmat(n,size(V,1),1),V-repmat(p,size(V,1),1),2);
    % check that poly is really coplanar
    if(sqrt(sum(dist_to_plane.^2))>1e-6)
